...
let configuration = prettier_instance.getData();
    ...
    "Configuration":{
        click:function(){
            ...
            const config_window = new Window({
                ...
                content:`
                    <div class=section1>
                        <h1>Prettier<h1>
                        <div class=section1>
                            <textarea id=textarea_config>${JSON.stringify(configuration)}</textarea>
                        </div> 
            ...
            document.getElementById("save_config").onclick = function(){
                try{
                    configuration = JSON.parse(document.getElementById("textarea_config").value);
                }catch{
                    new Notification("Prettier","There were an error while parsing the configuration.");
                }
                
                prettier_instance.saveData(configuration);
            }
             document.getElementById("restart_config").onclick = function(){
                document.getElementById("textarea_config").value = JSON.stringify(default_config);
